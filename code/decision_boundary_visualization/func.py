...
import sys
# sys.path.append('F:/application/git/myProject/method3/ModelDiff/data')
...
def get_dataloader(dataset_id, datasets_dir='F:/application/git/myProject/method3/ModelDiff/data', split='train', batch_size=128, shuffle=True, seed=98, shot=-1):
    """
...
def get_two_model(pth1, pth2, classes):
    device_name = 'cuda' if torch.cuda.is_available() else 'cpu'
    device = torch.device(device_name)
    print("Using {}".format(device))
    ...
    # set steal models
    if 'steal' in pth1:
        steal_model_str = pth1.split('-')[-2]
        for model_arch in model_archs:
            if model_arch in steal_model_str:
                if 'res' in model_arch:
                    logging.info("Loading model:{} …………".format(model_arch))
                    model1 = eval(f'{model_arch}_dropout')(
                        pretrained=False,
                        num_classes=classes
                    )
                elif 'mbnet' in model_arch:
                    logging.info("Loading model:{} …………".format(model_arch))
                    model1 = eval(f'{model_arch}')(
                        num_classes=classes
                    )
    else:
        # set normal models
    ...
    # after loading models, set quantize models
    if 'quantize' in pth1:
        dtype = torch.qint8 if 'qint' in pth1 else torch.float16
        model1 = torch.quantization.quantize_dynamic(model1, dtype=dtype)
    ...
    # model2 = eval(f'resnet18_dropout')(
    #     pretrained=False,
    #     num_classes=classes
    # )
    # checkpoint2 = torch.load(pth2)
